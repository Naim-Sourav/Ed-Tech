...
প্রিভিউয়ের `/`-এ একটা **মেনু পেজ** আসে — সব প্রশ্ন পেজ, অধ্যায়ের তালিকা আর সিলেবাস
পেজের লিংকসহ। কারণ `dist/index.html` হলো SPA শেল, যেটার জন্য JS + Firebase/API লাগে;
ওটা খুললে প্রশ্ন দেখা যেত না। আসল SPA শেল দেখতে চাইলে `/index.html`, অথবা
`--spa-root` ফ্ল্যাগ দাও।
...
## ৭ক. কাজ করতে গিয়ে ধরা পড়া একটা আসল বাগ

ডাটাবেজের কিছু স্লাগে এমন ক্যারেক্টার আছে যা **অকার্যকর URL** বানায়:

| স্লাগ | সমস্যা |
|---|---|
| `10%-Na-2CO-3-দ্রবণের-মোলারিটি-কত` | `%`-এর পরে দুই hex অঙ্ক নেই → ভাঙা percent-escape |
| `...-Ni-2+-0-1M-\|\|Ag` | `\|` পাথে অনুমোদিত না |

এই ২৬১টি স্যাম্পলেই ৩টি এমন ছিল — ৫০,৩৪৩ প্রশ্নে সম্ভবত কয়েকশো। এসব `<loc>`
sitemap-এ অকার্যকর ছিল, তাই Google ওগুলো **পড়েই ফেলে দিত** (চুপচাপ, কোনো এরর ছাড়াই)।

ফিক্স: `encodeSlug()` — শুধু যে ক্যারেক্টারগুলো সত্যিই অবৈধ সেগুলো percent-encode করে,
বাংলা টেক্সট IRI হিসেবে পাঠযোগ্য রাখে (আগের sitemap-এও তাই ছিল)। ডিস্কের ফোল্ডারের নাম
**কাঁচা স্লাগ-ই থাকে**, কারণ সার্ভার রিকোয়েস্ট পাথ ডিকোড করে ওই নামেই ফাইল খোঁজে।
...
- `sitemap.xml` XML-parsed: ৪৬৯টি URL (২৬১ প্রশ্ন + ৯৭ তালিকা + ১১০ সিলেবাস + হোম),
  **০টি অকার্যকর URL** (প্রতিটি `<loc>` `new URL()` দিয়ে পার্স করা + অবৈধ ক্যারেক্টার স্ক্যান)
- সব জেনারেট হওয়া পেজের ১,৩৮৭টি internal `/q/` লিংক স্ক্যান — ০টি অবৈধ
- `encodeSlug()` ইউনিট টেস্ট ৫/৫ পাস
