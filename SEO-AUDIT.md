...
## ৭. আপডেট (২০২৬-০৯-২১) — বিগত বছরের ভর্তি প্রশ্নপত্র: `/admission-questions/…`

ধাপ-৩-এর টেবিলে প্রস্তাবিত `/admission/medical/` + `/hsc-question-bank/` পেজগুলো এখন **একটি ইঞ্জিন** হিসেবে বাস্তবায়িত — HSC সিলেবাস পেজ যে রেসিপিতে র‍্যাংক করছে (হাব → বিষয় → অধ্যায়), ঠিক সেই রেসিপিতে (হাব → প্রতিষ্ঠান → সেশন)।

### ডেটা কোথা থেকে আসে
- প্রশ্নব্যাংকে প্রতিটি প্রশ্নের `tags` অ্যারেতে এক্সাম-সেশন ট্যাগ থাকে: `Medical '21-22`, `DU-A '25-26`, `Dental '23-24` …
- API-তে `GET /admin/questions?board=<ট্যাগ>` এই অ্যারের উপর (partial match) ফিল্টার করে। জেনারেটর প্রতিটি প্রতিষ্ঠানের জন্য `board=Medical '` দিয়ে সব সেশন একবারে টেনে আনে, তারপর exact ট্যাগ ধরে প্রশ্নপত্রে ভাগ করে। **কিছুই হার্ডকোড নয়** — DB-তে নতুন সেশন ট্যাগ করলেই পরের বিল্ডে পেজ তৈরি হবে।
- বান্ডল করা `data/medical_24_25_questions.json` / `gst_a_23_24_questions.json` API-র সাথে মার্জ হয় (প্রশ্নের টেক্সট ধরে ডুপ্লিকেট বাদ) — API ডাউন থাকলেও অন্তত এই দুটি পেপার থাকে।
- প্রতিষ্ঠানের নাম/স্লাগ/পরীক্ষার ধরন/FAQ এক জায়গায়: `data/admissionExams.ts` (জেনারেটর + অ্যাপ দুটোই এটা ব্যবহার করে)।

### যে পেজগুলো তৈরি হয়
| URL | কী আছে | priority |
|---|---|---|
| `/admission-questions/` | সব প্রতিষ্ঠান, ক্যাটাগরি অনুযায়ী; সাম্প্রতিক প্রশ্নপত্র; পরিসংখ্যান | 0.9 |
| `/admission-questions/medical/` | সেশন-কার্ড (প্রশ্ন সংখ্যা, বিষয় বণ্টন), পরীক্ষার ধরন টেবিল, **যে অধ্যায় থেকে বেশি প্রশ্ন এসেছে** (আসল ডেটা থেকে), FAQ + FAQPage JSON-LD | 0.8 |
| `/admission-questions/medical/2021-22/` | "মেডিকেল ভর্তি পরীক্ষা ২০২১-২২ — প্রশ্ন ও সমাধান": বিষয় অনুযায়ী সাজানো পুরো প্রশ্নপত্র, প্রতিটি প্রশ্নে অপশন + সঠিক উত্তর + ব্যাখ্যা (MathJax), উত্তর লুকিয়ে নিজে সলভ করার টগল, prev/next সেশন, অ্যাপে টাইমার-সহ পরীক্ষার CTA; BreadcrumbList + LearningResource JSON-LD | 0.8 |
| `/q/<slug>/` | আগের মতোই; এখন প্রশ্নটি যে পরীক্ষায় এসেছে তার লিংক + "একই পরীক্ষার আরও প্রশ্ন" (ইন্টারনাল লিংক গ্রাফ শক্ত হলো) | 0.6 |

অ্যাপের ভেতরেও একই URL কাজ করে (`components/PastPaperPage.tsx`, `noindex`, canonical → স্ট্যাটিক পেজ): স্ট্যাটিক পেজ না থাকলে (যেমন নতুন সেশন) 404.html → SPA → লাইভ DB থেকে লোড।

### সাথে যা যা করা হয়েছে
- `robots.txt`: `Allow: /admission-questions/`, `Allow: /q/`; `sitemap.xml`-এ সব নতুন URL; `public/sitemap.xml` fallback-এ হাব।
- `index.html` `<noscript>`-এ হাব + মেডিকেল পেজের লিংক; HSC সিলেবাস হাবে ক্রস-লিংক; স্ট্যাটিক পেজের ফুটারে হাব লিংক।
- ল্যান্ডিং পেজ: নতুন "বিগত বছরের প্রশ্ন" সেকশন (ফিচার্ড: মেডিকেল ২০২৪-২৫ + প্রতিষ্ঠানের চিপ), ফুটারের সব `#top` ডেড-লিংক এখন আসল পেজে যায় (প্রশ্ন সমাধান / সিলেবাস কলাম), নেভবারে "প্রশ্ন সমাধান"।
- অ্যাপ: `fetchQuestionsByExamRefAPI` এখন `/quiz/past-paper/` খালি দিলে ট্যাগ-ফিল্টারে ফলব্যাক করে — প্রশ্নব্যাংকের "সকল সেশন → Medical '21-22" ফ্লো এখন আসল প্রশ্ন দেখায়।
- Google ২০২৬-এ Practice-problem (Quiz) ও FAQ rich result বন্ধ করেছে; তাই ভারী Quiz markup যোগ করা হয়নি — BreadcrumbList/ItemList/LearningResource-ই যথেষ্ট, কনটেন্ট নিজেই মূল সিগন্যাল।

### ডিপ্লয়ের পর করণীয়
1. GSC → URL Inspection → `/admission-questions/` ও `/admission-questions/medical/` "Request indexing"; sitemap আবার সাবমিট।
2. Facebook/Telegram-এ "মেডিকেল ভর্তি পরীক্ষা ২০২৪-২৫ প্রশ্ন সমাধান" লিংক শেয়ার (প্রথম ব্যাকলিংক + রেফারেল)।
3. DB-তে যত বেশি সেশন ট্যাগ করা যায় (Medical '15-16 … '25-26, Dental, DU-A…) — প্রতিটি সেশন = একটি নতুন র‍্যাংকযোগ্য পেজ। ১০টির কম প্রশ্নের সেশন পেজ হয় না (thin content এড়াতে)।
