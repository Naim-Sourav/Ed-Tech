...
import { EASE_OUT_EXPO, Logo } from './ui';
...
/**
 * Fixed glass navigation for the public landing page.
 * Section links are plain anchors (same-document), CTAs go through
 * react-router via the `onLoginClick` handler passed down from App.tsx.
 */
const LandingNav: React.FC<LandingNavProps> = ({ onLoginClick }) => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Desktop links */}
          <nav className="relative z-10 hidden items-center gap-1 lg:flex" aria-label="প্রধান মেনু">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
              ...
              <ul className="flex flex-col">
                {navLinks.map((link, i) => (
                  ...
                  >
                    ...
                    >
                      {link.label}
                      <span className="text-sm font-medium text-mist">{link.en}</span>
                    </a>
                  </motion.li>
                ))}
              </ul>
