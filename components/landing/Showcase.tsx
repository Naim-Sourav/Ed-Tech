...
import { EASE_OUT_EXPO, Reveal, SectionTag, toBn } from './ui';

/** Tabbed product tour: HSC · Admission · GST tracks. */
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-end">
          <Reveal className="max-w-2xl">
            ...
            <h2 className="mt-5 font-display text-[34px] font-bold leading-[1.05] tracking-[-0.02em] text-white sm:text-[52px]">
              এক অ্যাপ।
              <br />
              প্রতিটা লড়াইয়ের <span className="text-lime">আলাদা ট্র্যাক।</span>
            </h2>
          </Reveal>

          {/* Tabs */}
          <Reveal delay={0.15}>
            <div className="flex rounded-full bg-white/8 p-1.5 ring-1 ring-white/10" role="tablist" aria-label="প্রস্তুতি ট্র্যাক">
              {showcaseTabs.map((t) => (
                <button
                  key={t.id}
                  type="button"
        ...
        <div className="mt-14 grid items-center gap-12 lg:grid-cols-[1fr_1.08fr] lg:gap-16">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h3 className="text-balance font-bangla text-[27px] font-bold leading-tight tracking-[-0.01em] text-white sm:text-[36px]">
                {tab.headline}
              </h3>
              ...
              <ul className="mt-8 space-y-5">
                {tab.bullets.map((b, i) => (
                  <motion.li
                    ...
                    transition={{ delay: 0.15 + i * 0.1, duration: 0.55, ease: EASE_OUT_EXPO }}
                    className="flex gap-4"
                  >
                    <span className="mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-lime/15 text-lime ring-1 ring-lime/25" aria-hidden="true">
            ...
            >
              <div className="glass-dark relative overflow-hidden rounded-[26px] p-6 shadow-[0_44px_90px_-30px_rgba(0,0,0,0.7)] sm:p-8">
                    ...
                    <div key={r.name}>
                      ...
                      <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10">
                        <motion.div
                          ...
                          transition={{ duration: 1.1, delay: 0.25 + i * 0.14, ease: EASE_OUT_EXPO }}
                          className={`h-full rounded-full ${r.accent ? 'bg-gradient-to-r from-brand-bright to-lime' : 'bg-white/35'}`}
                        />
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-8 grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="flex flex-col justify-center rounded-2xl bg-white/[0.07] p-5 ring-1 ring-white/10">
                    ...
                    <p className="mt-1.5 text-[12.5px] leading-relaxed text-white/50">অ্যানালিটিক্স থেকে লাইভ আপডেটেড</p>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <span className="text-[12.5px] font-bold text-white">
                  <span className="font-display tabular-nums text-lime">{toBn('5230')}</span> জন এখন অনলাইনে
                </span>
              </motion.div>
            </motion.div>
          </AnimatePresence>
