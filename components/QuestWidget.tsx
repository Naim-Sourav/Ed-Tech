...
const QuestWidget: React.FC<QuestWidgetProps> = ({ quests, onQuestUpdate }) => {
  ...
  const getIcon = (iconName?: string) => {
      switch(iconName) {
          ...
          case 'FileCheck': return <FileCheck size={18} className="text-purple-700 dark:text-purple-400" />;
          case 'Clock': return <Clock size={18} className="text-purple-500" />;
          case 'Swords': return <Swords size={18} className="text-purple-700 dark:text-purple-400" />;
                    ...
                    <div key={quest.id} className={`p-3 rounded-xl border transition-all ${quest.claimed ? 'bg-gray-50 dark:bg-black border-gray-100 dark:border-zinc-800 opacity-60' : 'bg-white dark:bg-zinc-900 border-gray-200 dark:border-zinc-800 hover:border-primary/30'}`}>
                        <div className="flex justify-between items-start mb-2">
                            ...
                            {quest.claimed ? (
                                ...
                                <span className="text-xs font-bold text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 px-2 py-1 rounded">+{quest.reward} Pts</span>
                            )}
                        </div>
